...
class FlinkJdbcPreparedStatement(val sql: String) extends PreparedStatement {
  ...
  override def executeQuery(): ResultSet = {
    ...
    new FlinkJdbcResultSet(sparkSession.sqlQuery(buildSql), this)
  }

  override def executeUpdate(): Int = {
    println(s"[DRIVER] executing sql $buildSql")
    ETLFlinkSession.batchEnv.executeSql(buildSql)
    //sparkSession.executeSql(buildSql)
    0
  }
  ...
  override def execute(sql: String): Boolean = {
    this.resultSet = new FlinkJdbcResultSet(sparkSession.sqlQuery(buildSql), this)
    true
  }

  override def getResultSet: ResultSet = new FlinkJdbcResultSet(sparkSession.sqlQuery(buildSql), this)
